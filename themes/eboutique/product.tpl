...
<div id="primary_block" class="primary_block clearfix">
	...
	<div class="clearfix">
	<!-- left infos-->
	<div id="pb-left-column" class="pb-left-column">
		<!-- product img-->
		<div id="image-block" class="image-block prod-image">
		{if $have_image}
			<div id="view_full_size" class="view_full_size">
				...
				<div class="span_link">{l s='View full size'}</div>
			</div>
		{else}
	...
	<!-- right infos-->
	<div id="pb-right-column" class="pb-right-column">
		<h1 class="prod-title">{$product->name|escape:'htmlall':'UTF-8'}</h1>
        
            <div id="product_reference" class="product_reference prod-ref">
                <span class="label_reference">{l s='Reference:'} </span>
                <span class="editable_reference">{$product->reference|escape:'htmlall':'UTF-8'}251112200015</span>
            </div>
        
		{if $product->description_short OR $packItems|@count > 0}
		<div id="short_description_block" class="prod-short_desc">
			{if $product->description_short}
				<div id="short_description_content" class="short_description_content rte align_justify">{$product->description_short}</div>
			{/if}
		...
		<form id="buy_block" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0}class="hidden"{/if} action="{$link->getPageLink('cart')}" method="post">
			...
			<div class="product_attributes clearfix">
				{if isset($groups)}
				<!-- attributes -->
				<div id="attributes" class="attributes clearfix">
				{foreach from=$groups key=id_attribute_group item=group}
					{if $group.attributes|@count}
                    	<div class="clearfix new_attribute {$group.group_type}">
                        	<div class="content_attribute right">
			...
			<!-- quantity wanted -->
            <div class="clearfix new_attribute"{if (!$allow_oosp && $product->quantity <= 0) OR $virtual OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
            	<div class="content_attribute right">
            ...
            <div class="clearfix new_attribute"{if $product->minimal_quantity <= 1 OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
            	<div class="content_attribute right">
                    <div id="minimal_quantity_wanted_p" class="minimal_quantity_wanted_p">
			...
			<!-- availability -->
            <div class="clearfix new_attribute"{if ($product->quantity <= 0 && !$product->available_later && $allow_oosp) OR ($product->quantity > 0 && !$product->available_now) OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
            	<div class="content_attribute right">
            ...
            <div class="clearfix new_attribute"{if $product->quantity <= 0} style="display: none;"{/if}>
            	<div class="content_attribute right">
                    {if ($display_qties == 1 && !$PS_CATALOG_MODE && $product->available_for_order)}
                    <div id="pQuantityAvailable" class="pQuantityAvailable">
            ...
            <div class="clearfix new_attribute" {if ($product->quantity > $last_qties OR $product->quantity <= 0) OR $allow_oosp OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none"{/if}>
            	<div class="content_attribute right">
					<div class="warning_inline" id="last_quantities" >{l s='Warning: Last items in stock!'}</div>
            	</div>
            </div>
		</div>
		...
		<div class="content_prices clearfix">
			...
			<div class="price">
				...
				{if $priceDisplay >= 0 && $priceDisplay <= 2}
					<span class="from">{l s='from'}</span> <span id="our_price_display" class="price">{convertPrice price=$productPrice}</span>
			...
			{else}
				<p id="add_to_cart" class="buttons_bottom_block add_to_cart">
					<span></span>
					<input type="submit" name="Submit" value="{l s='Add to cart'}" class="exclusive buttons_add_to_cart" />
				</p>
			{/if}
    ...
    <div class="clearfix">
    <!-- usefull links-->
		<ul id="usefull_link_block" class="usefull_link_block clearfix">
